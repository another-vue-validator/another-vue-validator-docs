<template>
  <div class="layout-chapter">
    ...
    <hr/>
    <div id="built_in_rules" class="section-title">
    ...
    <div id="custom_rule" class="section-title">
      <div>Custom Rule</div>
    </div>
    <div class="section-content">
      ...
      <p>
        ...
        the callback function is expected to return an error message (or promise, see <a href="#async_validation">Async
        Validation</a>) if validation fails.<br/>
      ...
      <p>
        ...
        this method is internally used by <span class="code">Validator.required()</span> method to determine if a field
        value is empty,
        since most validation rules apply only when the value is NOT empty, this method is also exposed as public API so
        that it can be used by custom code.
      </p>
    </div>
    <div id="cross_field_validation" class="section-title">
    ...
    <div class="section-content">
      ...
      <p>
        ...
        First of all, its value should match the <span class="code">password</span> field, this means <b>cross field
        validation</b>.
        Secondary, when the form is not already submitted, it should not be validated if user haven't touched it,
        otherwise we will see validation error message for <span class="code">repeat</span> field when user is just
        typing in <span class="code">password</span> field but haven't touch the <span class="code">repeat</span> field
        yet!<br/>
  ...
  if (this.submitted || this.validation.isTouched('repeat')) {
    return Validator.value(repeat).required().match(password);
      ...
      <p>
        In the validators config, we use <span class="code">'repeat, password'</span> as key, so that the library would
        watch these 2 fields for us,
        note that now we expect the library to provide values of both <span class="code">repeat</span> and <span
        class="code">password</span> in the validator function. <br/>
        In the code, we only perform validation when the form is already submitted or the <span
        class="code">repeat</span> field is already touched by user.
        We also enforce that the value of <span class="code">repeat</span> field should match the value of <span
        class="code">password</span> field.
      </p>
      <div class="note">
        We use <span class="code">ValidationBag.isTouch()</span> method to detect if a field is touched by user,
        check out <a href="#r_validation_bag">ValidationBag API</a> to find more details about this method and other
    ...
    <div class="section-content">
      ...
      <p>
        ...
        There are also some differences. First, we use <span class="code">v-if="fruit === 'other'"</span> to make the
        text field visible only when "Other fruit (please specify)" option is selected.
      ...
      <div class="note">
        ...
        thus we use use it as opposed to <span class="code">'fruit'</span> when checking for error to display: <span
        class="code">validation.hasError('other')</span>.
      </div>
    </div>
    <div id="async_validation" class="section-title">
  ...
  if (!Validator.isEmpty(value)) {
    return Promise.delay(1000)
      ...
      <p>
        Here we simply delay the execution for 1 second, in the real world, you probably will make an ajax request to
        server.
      </p>
      <div class="note">
...
&lt;i v-if=&quot;domain &amp;&amp; validation.isPassed('domain')&quot; class=&quot;text-success fa fa-check-circle&quot;&gt;&lt;/i&gt;</code></pre>
      <p>
        In the template HTML, we use <span class="code">ValidationBag.isValidating()</span> to show an animating spinner
        when async validation is in progress,
    ...
    <div class="section-content">
      ...
      <ul>
        ...
        <li>The async validation is activated every time user hits the keyboard, this would cause performance issue if
      ...
      <div class="note">
        The <span class="code">cache</span> option can also take a <span class="code">'all'</span> string, which is
        equivalent to <span class="code">true</span>,
    ...
    <div id="custom_component" class="section-title">
      <div>Custom Component</div>
      </div>
    <div class="section-content">
      <!--      <DemoWithCode :components="['CustomComponentExample', 'CheckboxGroup']"/>-->
      <p>
        The library can work with any custom component as long as the component supports <span
        class="code">v-model</span>.
        The above example demonstrates using a custom <span class="code">CheckboxGroup</span> component that wraps the
        <a href="http://icheck.fronteed.com/">iCheck</a> library.
      </p>
    </div>
    <div id="dynamic_form" class="section-title">
      <div>Dynamic Form</div>
      </div>
    <div class="section-content">
      <!--      <DemoWithCode :components="['DynamicFormExample', 'DynamicForm']"/>-->
      <p>
        To implement dynamic form (form created at runtime), move the form related HTML and codes into a separated child
        component and expose a validate method,
        this would result in multiple <a href="#r_validation_bag">ValidationBag</a> instances attached to the child
        components.<br/>
        You can then invoke the validate method from parent to trigger validation logic.
      </p>
    </div>
    <div id="custom_error_message" class="section-title">
      <div>Custom Error Message / Localization</div>
      </div>
    <div class="section-content">
      <div class="note">
        ...
        you will have to update your templates according to the changes <a target="_blank"
                                                                           href="https://github.com/semisleep/simple-vue-validator/commit/500402134346b8a014cc961b55dc1bd2189f59fb#diff-8f4411d1af5a3b8a78502627925694d8">here</a>.
      </div>
    </div>
    ...
    <div class="section-content">
      ...
      <p>
        Use <span class="code">SimpleVueValidator.extendTemplates()</span> method (or <span class="code">Vue.use(SimpleVueValidator, {templates: {...})</span>)
        to override the default error messages.
        This can also be used to provide localized messages to the library.
      </p>
      <div class="note">
        See <a target="_blank" href="https://github.com/semisleep/simple-vue-validator/blob/master/src/templates.js">templates.js</a>
        for all the text templates you can replace.
      </div>
    </div>
    ...
    <div class="section-content">
      ...
      <p>
        Use <span class="code">Validator.create({templates: ...})</span> to create a new <span
        class="code">Validator</span> with customized error messages.
        Overriding the error messages template this way will have the scope limit to the newly created <span
        class="code">Validator</span>, so it won't affect
        the global settings.
      </p>
    </div>
    ...
    <div class="section-content">
      ...
      <p>
        For all the built-in rules, you can pass a custom message as the last argument to specify the error message for
